package com.tungnui.abccomputer.dialog
...
import com.tungnui.abccomputer.network.helper.RequestCoupons
...
class DiscountDialogFragment : DialogFragment() {
    ...
    private val isRequiredFieldsOk: Boolean
        get() {
            ...
            if (discount_code_input_wrapper.editText == null || discount_code_input_wrapper.editText?.text.toString().equals("", ignoreCase = true)) {
                ...
                discount_code_input_wrapper.error = "Vui lòng nhập mã giảm giá"
            } else {
    ...
    private fun sendDiscountCode(discountCodeInput: EditText?) {
        ...
        if (requestListener != null)
            requestListener?.requestSuccess(discountCodeInput?.text.toString());
    ...
    interface RequestListener {
        fun requestSuccess(couponCode: String)
    }
